...
# Models with external MTP speculative decoding draft layers
models_has_mtp() {
    local name="${1:-$LODGE_MODEL}"
    local key
    ...
    case "$key" in
        gemma4-e2b-inst)
            ...
            ;;
        gemma4-e4b-inst)
            if [ -f "${LODGE_DIR:-/workspace}/.george/models/mtp-gemma-4-E4B-it.gguf" ]; then
                return 0
            fi
            return 1
            ;;
        gemma4-12b-inst)
            if [ -f "${LODGE_DIR:-/workspace}/.george/models/mtp-gemma-4-12b-it.gguf" ]; then
                return 0
            fi
            return 1
            ;;
