...
INTERLOCK_JSON
        ...
        if [ -n "$cmd" ]; then
            ...
            if [ $exit_code -eq 0 ]; then
                ...
                # ── READ FILE & VISION TRACKING ───────────────
                # Keep track of files read or images analyzed during
                # this task and save their contents/descriptions into
                # macro_memory.json's read_context.
