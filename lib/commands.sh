...
"commands":{
  ...
  "RESEARCH & MEMORY (TOOLS)":{
    ...
    "/web":{"syntax":"/web <action> <query|url>","desc":"Web search, fetch/scrape, and image extraction",
      ...
      "rules":["search=QUERY (keywords), fetch/scrape=URL — NEVER swap","/web fetch and scrape both return structured JSON with content text, images[], and links[]","1 search + 1-2 fetches/scrapes is enough — do NOT fetch every result","scrape/fetch returns {url,title,content,images[],links[]} — pass images[] URLs to /vision","LOCAL FILES: NEVER use /web fetch or /web scrape on local files or relative paths — use /read for text files, /vision for images","ONE URL PER COMMAND — never put multiple URLs in one /web call. The URL must be the LAST token — nothing after it."],
      ...
      "chains":["Text research: /web search <topic> -> /web fetch <url> -> summarize","Scrape workflow: /web search <topic> -> /web scrape <url> -> summarize","Image research: /web scrape <url> -> /vision <image_url_from_images[]>","Image search: /web images <query> -> /vision <image_url>","Image download workflow: /web scrape <url> -> /download <image_url_from_images[]> [destination] -> /vision <destination>","Deep page analysis: /web scrape <url> -> read content + /vision on each image"],
    ...
    "/download":{"syntax":"/download <url_or_path> [destination]","desc":"Download a file or copy a local file to the workspace.",
      "format_only_ex":["/download <url> [destination-path]"],"fill":{"<url_or_path>":"URL to download or local path to copy","[destination]":"optional destination file path (defaults to source filename)"}},
    "/vision":{"syntax":"/vision <image> [prompt]","desc":"Analyze/describe an image using vision model. Accepts URLs directly (no /download needed) or local image file paths.",
      "notes":["Supports jpg/png/gif/webp/bmp","Pair with /web scrape to analyze images from web pages","Default prompt: describe image in detail (text, objects, people)"],
      "format_only_ex":["/vision <image> <prompt>"],
      "fill":{"<image>":"URL or local path to image (e.g. from /web scrape images[] array or local path from /download)","<prompt>":"what to analyze or describe"}},
