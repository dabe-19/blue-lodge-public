---
name: trowel
description: The Terminal Node. Marks the milestone as complete in {{STATUS_FILE_PATH}} and permanently severs the Autopilot loop.
...
<workflow>
1. Use the `read` tool to quickly check `{{CONTRACT_PATH}}` to see what was just finished.
2. Use the `edit` tool to update `{{STATUS_FILE_PATH}}` in the root directory. Move the current task from "The Active Board" down to "The Trowel (Completed Milestones)". Use this exact one-line template, no boilerplate, no theme-audit recap:
