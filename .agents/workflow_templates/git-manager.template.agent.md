...
- Every commit MUST be traceable to `@{{CONTRACT_PATH}}`.
...
## 2. Validate Scope
Read `@{{CONTRACT_PATH}}`. Compare the diff against the contract's stated scope. If unrelated files are included, HALT and tell the operator a split is needed.
...
Footer (optional): `Refs: @{{CONTRACT_PATH}}`
