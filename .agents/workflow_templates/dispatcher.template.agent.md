...
2. Read the specialist's workflow file in `.agents/workflows/` (e.g. `quartermaster.agent.md`, `core-specialist.agent.md`, etc.), adopt its persona/rules/workflow, and execute its tasks.
...
If Functional Verification is `yes`: read `.agents/workflows/tester.agent.md` to become the tester, run the full verification test suite (`{{TEST_CMD}}`), and load `/home/wsl-ops/blue-lodge/.agents/workflows/dispatcher.agent.md` using `view_file` to resume the dispatcher role once they pass successfully.
