...
- **Incremental Test Validation**: Run specific unit tests (`bash tests/run_all.sh test_optimizations test_secrets test_security`) incrementally after edits to ensure you don't accumulate test errors early in development.
- **Test Coverage Policy**: You MUST write new unit tests (in appropriate files in `tests/`) whenever you add new functionality to prevent regressions in this large Posix bash codebase. All tests must be integrated with `tests/framework.sh`.
- Run `bash tests/run_all.sh test_optimizations test_secrets test_security` to verify your changes incrementally.
