...
- `/core-specialist` — manages lib/*.sh and core logic
- `/commands-specialist` — manages commands/*.sh and slash commands
- `/ui-specialist` — manages lodge and terminal UI
- `/tests-specialist` — manages tests/ and test suites
...
- After EACH subagent returns, run the build gate: `N/A` — `none`. If red, HALT and report which layer broke it.
...
4. Run the build gate (`N/A`). Green → mark `completed`. Red → HALT.
